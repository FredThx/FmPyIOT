...
class Etuve:
    ...
    def set_iot(self, iot:FmPyIotWeb):
        ...
        self.iot.add_topic(Topic("./temperatures", read=self.get_temperatures, send_period=30))

    @staticmethod
    def decode_rom(rom:str)->str:
        '''Convertit un rom hex  en string int'''
        return ":".join([h[2:].upper() for h in rom.split(',')])

    async def get_temperatures(self, **kwargs)->dict:
        if self.ds:
            temperatures = await self.ds.read_all_async()
            temperatures = {self.decode_rom(rom):temp for rom,temp in temperatures.items()}
            for rom in temperatures:
                if rom in self.params['temperatures_offsets']:
                    temperatures[rom] += float(self.params['temperatures_offsets'][rom])
                else:
                    self.params['temperatures_offsets'][rom] = 0.0
                    self.iot.set_param('etuve', self.params)
            return temperatures
        else:
            return {}
...
iot = FmPyIotWeb(
    ...
    mqtt_base_topic = "OLFA/LABORATOIRE/ETUVE",
    watchdog=100,
    sysinfo_period = 600,
    led_wifi="LED",
    web=True,
    name = "Etuve Laboratoire",
